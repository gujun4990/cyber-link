...
!define AUTOSTART_ENTRY_NAME "CyberLink"
...
Section Install
  ...
  ; Enable autostart by default and keep the command line identical to the in-app toggle.
  WriteRegStr HKCU "${AUTOSTART_RUN_KEY}" "${AUTOSTART_ENTRY_NAME}" "$\"$INSTDIR\${MAINBINARYNAME}.exe$\" --autostart"
...
Section Uninstall
  ...
  DeleteRegValue HKCU "${AUTOSTART_RUN_KEY}" "${AUTOSTART_ENTRY_NAME}"
